...
form?.addEventListener('submit', async (e) => {
    ...
    const serviceOk = validateSelect('serviceType', 'serviceError');
    const goalOk    = validateSelect('goal', 'goalError');
    const dateOk    = validateDate();
    const timeOk    = validateSelect('preferredTime', 'timeError');
...
function buildPayload() {
    const serviceLabels = {
        ...
        full: 'Full Package (Training + Nutrition)',
        'plan-advice': 'Which plan should I pick?'
    };
    const goalLabels = {
    ...
    return {
        ...
        preferredSlot: `${dateVal} at ${timeVal}`,
...
async function submitReservation(payload) {
    ...
    throw lastErr || new Error('Could not submit reservation');
}
...
function showSuccess(payload) {
    ...
    successDetails.innerHTML = `
        ...
        <strong>Phone:</strong> ${escHtml(payload.phone)}<br>
        <strong>Service:</strong> ${escHtml(payload.serviceLabel)}<br>
        <strong>Goal:</strong> ${escHtml(payload.goalLabel)}<br>
        <strong>Preferred Slot:</strong> ${escHtml(payload.preferredSlot)}<br>
        ${payload.notes ? `<strong>Notes:</strong> ${escHtml(payload.notes)}` : ''}
    `.trim();
